...
class Competition(commands.Cog, name="competition"):
    ...
    async def createTeam(self, context):
        """
            Create new  team / Only team leader should create Team
        """
        user_id = context.author.id
        await context.trigger_typing()
        ...
        data["image"] = config.PROFILE_PICTURE_URL
        try:
        ...
        data["token"] = token
        r =  requests.post(config.API_URL+f"/api/participant/{participant_id}/team/create", json=data) 
        response = r.json()
        ...
        first_category = guild.get_channel(config.SPACES[0])
        ...
        if text_channels_number > 24:
            space_category = guild.get_channel(config.SPACES[1])
        
        team_role = get(guild.roles, name=f"{team_name}_{config.TEAM_NAME_SUFFIX_CHANNEL}")
        ...
        if team_role:
            ...
            await guild.create_role(name=f"{team_name}_{config.TEAM_NAME_SUFFIX_CHANNEL}")
            time.sleep(1)
            role = get(guild.roles, name=f"{team_name}_{config.TEAM_NAME_SUFFIX_CHANNEL}")
            overwrites = {
    ...
    async def help_mentor(self,context : commands.Context):
        ...
        if len(parts) == 2 and parts[1] == "space":
            role = get(guild.roles, name=config.MENTOR_ROLE)
            ...
            if(permission is not None and permission.read_messages is True and permission.send_messages is True):
                ...
                mentor_logs_channel = guild.get_channel(config.LOG_CHANNEL)
                embed = discord.Embed(
            ...
            raise UnknownError(message="Command can only be issued from a team channel")
    ...
    async def joinTeam(self, context,token:str):
        ...
        if res["status"] =='UNKNOWN_ERROR':
            ...
            log_channel = guild.get_channel(config.CHANNEL_LOG)
            embed = discord.Embed(
    ...
    async def removeMember(self, context,member:discord.Member):
        ...
        r =  requests.get(config.API_URL+f"/api/participant/{participant_id}") 
        res = r.json()
        team_name = res['team']['name']
        print(team_name)
        if currentCategory.id in  config.SPACES and  team_name !=None :
            await context.trigger_typing()
            user_id = context.author.id
            participant_id =verify_api(user_id)
            participant_id_to_delete =verify_api(member.id)
            r =  requests.delete(config.API_URL+f"/api/participant/{participant_id}/team/remove",
                            json={"member":participant_id_to_delete}) 
            res = r.json()
            if res["status"] =='UNKNOWN_ERROR':
                raise UnknownError()
            elif res["status"] == 'SUCCESS':
                team_role = get(member.roles, name=f"{team_name}_{config.TEAM_NAME_SUFFIX_CHANNEL}")
